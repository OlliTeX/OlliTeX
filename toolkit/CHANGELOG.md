...
## OlliTeX 0.1.0 (2026-09-13) — OlliTeX-adapted toolkit
Owner adaptation wave (2026-09-13):

- **Rebrand** Overleaf → OlliTeX across scripts, docs and defaults
  (display names, help text, container names, README + docs).
  Internal `OVERLEAF_*` env names are unchanged (server contract).
- **SaaS surface removed**: Server Pro mode and the git-bridge service
  are gone (CE github-sync stays — it lives in the OlliTeX image).
- **Hub-first config guidance**: branding/appearance/LLM/LanguageTool/
  notifications/templates are documented as hub-managed
  (/hub, /hub → Admin); toolkit files keep bootstrap-only vars.
- **ONE image-version file**: `lib/images.env` (server, mongo, redis,
  nginx, pinned Typst digest, TeX Live, LanguageTool) — all compose
  fragments + `bin/images` resolve from it.
- **compose_cep improvements integrated**: mongo replica-set
  auto-initiation IN the healthcheck + glibc SHSTK workaround, redis
  PONG healthcheck + AOF default, `host.docker.internal` mapping,
  nginx `/healthz` endpoint + healthcheck.
- **LanguageTool service** (`LANGUAGE_TOOL_ENABLED=true`) +
  `bin/languagetool-ngrams` (official per-language n-gram downloads:
  en de es fr nl — admin-selectable via `--languages` / overleaf.rc;
  layout matches the proven production compose_cep LT stack).
- **Operator TUI** `bin/ollitextui` (python3/curses): stack status +
  start/stop/restart/logs/images/pull/backup/doctor/n-grams — a thin
  layer over the unchanged bin/ scripts (functionality not lost).
- `bin/up` now pre-pulls the pinned Typst image (sandbox
  `--network none` contract) before first boot.
