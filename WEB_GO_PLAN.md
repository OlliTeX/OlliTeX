...
⇒ The api-profile surface is **NOT 100%**: the TPDS sync + project-new/resolve/folder-update slice is complete (+ gated), and `GET /internal/project/:id` is now DONE+gated (below); the /internal/* destructive×7 + editor privateApi (join, history/resync, verify doc-fetch) rows above remain. Do NOT flip web-api-overleaf until they are 1:1 (plus the web valid-basic gap).
