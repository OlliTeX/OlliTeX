...
⇒ The api-profile surface is **NOT 100%**: the TPDS sync + project-new/resolve/folder-update slice is complete (+ gated), `GET /perfTest` + `GET /internal/project/:id` + `POST /internal/project/:id/deactivate` are DONE+gated, and the **entire editor privateApi family** (`GET doc/:doc_id`, `POST .../changes/reject`, `POST .../join`, `POST .../history/resync`) is DONE+gated (below); **only the /internal/* destructive×6 rows remain** (plus the web valid-basic gap). Do NOT flip web-api-overleaf until they are 1:1 (plus the web valid-basic gap).
