...
- `/internal/*` (DESTRUCTIVE/cron — **6 remaining**): POST expire-deleted-projects-after-duration / expire-deleted-users-after-duration / project/:pid/expire-deleted-project, POST /internal/users/:uid/expire, GET /internal/project/:pid/zip, POST /internal/deactivateOldProjects → Node 401 (mounted+auth) / Go 404. These are STATEFUL (expire/archive/flush-DU); 200-paths are destructive and must NOT be live-driven on the shared e2e stack (one `deactivateOldProjects` probe deactivated 10 old never-opened fixtures — natural cron behavior; key fixtures stayed healthy). **`POST /internal/project/:pid/deactivate` is now DONE+gated** (below). (POST compile/pdf is Node 404 → Go matches, need not port.)
- Editor privateApi **COMPLETE**: (`GET /project/:pid/doc/:doc_id` DONE+gated; `POST .../doc/:doc_id/changes/reject` DONE+gated; **`POST /project/:pid/join` DONE+gated**; **`POST /project/:pid/history/resync` NOW DONE+gated** — all below.)
- `/status` ✓ (Go "web is alive (api)" == Node), `/health_check/{redis 200, mongo 500}` ✓ (both match).
⇒ **The api-profile surface is now 100% COMPLETE: every Node :3000-mounted router.mjs route now has a faithful Go handler and its safe, wire-verifiable contract is gated** — READ (doc-trio/details/personal_info/tag) + TPDS WRITE (createProject/resolveProject/folder-update/sync×6) + `GET /perfTest` + `GET /internal/project/:id` + `POST /internal/project/:id/deactivate` + the **entire editor privateApi family** (`GET doc/:doc_id`, `POST .../changes/reject`, `POST .../join`, `POST .../history/resync`) + the **/internal/* cron+PII+download ×6** (`expire-deleted-projects-after-duration`, `expire-deleted-users-after-duration`, `project/:projectId/expire-deleted-project`, `users/:userId/expire`, `project/:Project_id/zip`, `deactivateOldProjects`) — all DONE+gated (below; destructive/heavy success legs are best-effort + not driven, same standard as deactivate). **The only remaining cutover blocker is the web valid-basic gap.** Do NOT flip web-api-overleaf until the web valid-basic gap is resolved/accepted.

✅ **POST /internal/project/:project_id/deactivate — DONE + gated (2026-09-24):** Node privateApiRouter `InactiveProjectController.deactivateProject` → `InactiveProjectManager.deactivateProject(projectId)` = `flushProjectToMongoAndDelete` (DU `DELETE /project/{pid}`) → hooks → `DocstoreManager.archiveProject` (docstore `POST /project/{pid}/archive`) → `markAsInactive` (Mongo `projects.updateOne({_id},{active:false})`). Wire pinned Node :3000: unauth/wrong→**401** `Unauthorized` (challenge); bad-oid→**404** JSON VA `params.project_id`; valid-oid (ghost→no-op, **real→active:false verified**)→**200** text/plain `OK` (2B, X-Powered-By, weak ETag W/"2-nOO9…", no nosniff, +global CSP). Go `internalDeactivateHandler` (docapi.go) reuses `APIBasicGate401` + `delParamVA("project_id")` + `delHex24` + `apiText(r,200,"OK")`, best-effort `fireHTTP(DU DELETE /project/{pid})` + `fireHTTP(docstore POST /project/{pid}/archive)` + Mongo `active:false`. Node would 500 if a step throws (not exercised in this healthy stack — all deps return 2xx). Gate cases deact-unauth/deact-wrong/deact-badoid/deact-ghost (non-destructive: only the ghost leg reaches the 200 no-op); **uapi now 103 cases diffs=0**.
...
✅ **POST /project/:Project_id/join — DONE + gated (2026-09-24):** Node EditorRouter (privateApiRouter **only** → APIOnly; the web profile skips it) → `EditorHttpController.joinProject` ("called by the real-time API") → `_buildJoinProjectView` → `ProjectEditorHandler.buildProjectModelView` (+ `AuthorizationManager.getPrivilegeLevelForProjectWithProjectAccess` + `CollaboratorsGetter` flags). Non-site-admin privilege path (this CE stack has no site admin): owner_ref→owner, collablator_refs→readAndWrite, reviewer_refs→review, readOnly_refs→readOnly, (tokenBased) token refs, then legacy public; anon→public/token (private→NONE). Wire pinned Node :3000 (full-header):
  - unauth/wrong → **401** `Unauthorized` (challenge, `APIBasicGate401`);
  - `params.Project_id` not hex24 → **404 JSON** VA `Invalid Mongo ObjectId at \"params.Project_id\"`;
  - body invalid → **400 JSON** VA: missing userId → `Invalid input: expected string, received undefined at \"body.userId\" or Invalid input: expected \"anonymous-user\" at \"body.userId\"`; non-string userId → `expected string, received <type> at \"body.userId\" or …\"anonymous-user\"…`; invalid-oid string → `Invalid Mongo ObjectId at \"body.userId\"`; unknown key → `Unrecognized key: \"<k>\" at \"body\"`;
  - param+body both bad → **404** (param precedence) `; `-joined, param first;
  - ghost project → **404** text/plain `Not Found` (`details404Plain`);
  - access NONE (non-member anon, private) → **403** text/plain `Forbidden`;
  - granted → **200 JSON** `{project,privilegeLevel,isRestrictedUser,isTokenMember,isInvitedMember}` — the `project` model is a **fixed-key-order** build (undefined dropped): `_id,name,rootFolder:[tree],publicAccesLevel,dropboxEnabled,grammarPicky,deletedByExternalDataSource,owner,members,invites,editAccessRequests|myAccessRequest,features,trackChangesState`; `features = _.defaults(owner.features,DEFAULTS)` (owner keys in stored order + missing DEFAULTS appended in DEFAULTS order) then `referencesSearch ||= references`, `mendeley ||= references`; P4.13 owner = **1421B** byte-pinned (owner user-model `_id,first_name,last_name,email,privileges:"owner",signUpDate`, 16-key features, trackChangesState:false).
  Go: new `joinview.go` — `joinHandler` (validator = `apReadBody`+`apUnknown`+`delHex24`+`apType`, param-first precedence), privilege/flags `joinPrivilegeForUser`/`joinInvitedMember`/`joinTokenMember`/`joinRestricted`, faithful order-preserving builders `joinProjectModelView`/`joinFolderModel`/`joinFileModel`/`joinDocModel`/`joinUserModel`/`joinFeatures`/`joinAccessRequestsView` + owner/invited-member user loads, serialized via `core.OrderedD`/`core.WriteOrderedValue`. Site-admin, public-token, non-owner `myAccessRequest`, and invited-member branches are source-faithful but inert in this stack (gate exercises the P4.13 owner 200 + all error boundaries). 11 gate cases `join-*`; **uapi now 114 cases diffs=0** (full-header match incl. the 1421B 200). web regression u1/u103r/p413 green.

✅ **POST /project/:Project_id/history/resync — DONE + gated (2026-09-24):** Node HistoryRouter (privateApiRouter **only** → APIOnly, `requirePrivateApiAuth`) → `HistoryController.resyncProjectHistory` → `ProjectEntityUpdateHandler.resyncProjectHistory` (wrapWithLock: getProject + overleaf check → `_checkFiletree` → **DU POST /project/{pid}/history/resync** [+ setHistoryRangesSupport]). Wire pinned Node :3000 (full-header, **non-204 legs only** — the 204 is heavy/cross-service and NOT live-driven in the gate; Go does a best-effort DU resync, same standard as deactivate):
  - unauth/wrong → **401** `Unauthorized` (challenge, `APIBasicGate401`);
  - `params.Project_id` not hex24 → **404 JSON** VA `Invalid Mongo ObjectId at \"params.Project_id\"`;
  - body invalid → **400 JSON** VA, joined `; ` in zod-declaration order [historyRangesMigration, resyncProjectStructureOnly, unknown-key]: enum → `Invalid option: expected one of \"forwards\"|\"backwards\" at \"body.historyRangesMigration\"` (`.optional()` but explicit **null also fails**); bool → `Invalid input: expected boolean, received \u003ctype\u003e at \"body.resyncProjectStructureOnly\"`; unknown → `Unrecognized key: \"\u003ck\u003e\" at \"body\"`;
  - param+body both bad → **404** (param precedence) `; `-joined, param first;
  - **ghost project → 500 text/plain `Internal Server Error` (21B — Node quirk: getProject→null→`project.overleaf` TypeError→throw→500, distinct from details' 404)**;
  - **overleaf.history.id absent → 404 text/plain `Not Found`** (ProjectHistoryDisabledError → sendStatus(404)) — the uapi-wire seed has overleaf=null, so this is the stable gate fixture;
  - history-enabled → **204** (Node sendStatus(204), weak ETag over "No Content") — Go best-effort `fireHTTP(DU POST .../history/resync)` + `ETag(EtagWeakBody("No Content"))`+`NoContent()`; **NOT gated** (heavy, needs a history-enabled project + cross-service).
  Go: new `historyresync.go` — `resyncHistoryHandler` (param-first precedence + historyRangesMigration enum + resyncProjectStructureOnly bool + `apUnknown`), `dpath(pd,"overleaf","history","id")` for the enabled check, `apiText(r,500,"Internal Server Error")` for the ghost 500 quirk, `apiText(r,404,"Not Found")` for history-disabled. 11 gate cases `rsync-*`; **uapi now 125 cases diffs=0**. web regression u1/u103r/p413 green.

✅ **/internal/* cron + PII + download ×6 — DONE + gated (2026-09-24):** Node router.mjs privateApiRouter (basic-auth, api-only). Wire pinned Node :3000 (2026-09-24); SAFE legs gated, DESTRUCTIVE/HEAVY success legs best-effort in Go + NOT driven (same standard as deactivate + history/resync-204). New `internalops.go` (6 handlers + `iopProfile`/`internalBulkGate`):
  - **all six** → unauth/wrong → **401** `Unauthorized` (challenge, `APIBasicGate401`).
  - **POST /internal/project/:projectId/expire-deleted-project** (→ ProjectDeleter.expireDeletedProject): bad projectId → **404 JSON** VA `params.projectId`; **ACTIVE project → 200 text/plain `OK`** (Node only deletes a leftover deletedProject record — a NO-OP when none, i.e. the uapi-wire seed: gate-able + non-destructive); **GHOST (no project, no deletedProject record) → 404 text/plain `Not Found`** (Node ProjectDeleter.NotFoundError); deleted-project-present → 200 (destructive best-effort: DU delete + record delete; **NOT gated**).
  - **POST /internal/users/:userId/expire** (→ UserDeleter.expireDeletedUser, 204): bad userId → **404 JSON** VA `params.userId`; valid → **204** (Node fires hook + deletes onboarding + redacts deletedUser.user — side-effecting → **NOT gated**; no record → Node 500 TypeError).
  - **GET /internal/project/:Project_id/zip** (→ ProjectDownloadsController.downloadProject): bad Project_id → **404 JSON** VA `params.Project_id`; valid → **200** zip (HEAVY, non-deterministic bytes → **NOT gated**; Go returns a minimal valid zip + correct content-type/disposition as a best-effort placeholder); ghost → 404 (Node wire unstable → **NOT gated**).
  - **POST /internal/expire-deleted-projects-after-duration** (bulk ProjectDeleter, 200): no params → 401 gated; 200 **destructive bulk** (stack has 564 deletedProjects) → **NOT gated** (Go best-effort bounded pass → 200).
  - **POST /internal/expire-deleted-users-after-duration** (bulk UserDeleter, 200): no params → 401 gated; 200 destructive bulk (111 deletedUsers) → **NOT gated**.
  - **POST /internal/deactivateOldProjects** (bulk InactiveProjectManager, 200): no params → 401 gated; 200 deactivates old active projects → **destructive, NOT gated** (Go best-effort → 200).
  **17 gate cases `exp-*`/`zip-*`/`bulk-*`/`deactold-*`; uapi now 142 cases diffs=0** (full-header match). web regression u1/u103r/p413 green; go build/vet/test/gofmt clean. ⇒ **api-profile 100%: the only remaining cutover blocker is the web valid-basic gap.**
...
- **KEY CROSS-DEP (the only one) — RESOLVED (Slice A, commit `bccf426d5e`):** the shared `Tag`/`Source`/notification-prefs types were TYPE-ONLY imports from the Node backend (erased at webpack compile, needed only for `tsc`). The 3 self-contained modules (no runtime values; `isPrivilegeUpgrade`'s impl is `PrivilegeLevels.mjs`, untouched) are relocated into the kept **`services/web/types/api/`** home (`tags.d.ts`, `authorization.d.ts`, `notifications.d.ts`) with re-export shims at the old paths → the 24+ type importers keep resolving, go build+vet green, no dangling refs. `app/` now has zero frontend type deps and is junk-able.
...
- **B.** Per-module backend junk (see MODULE SEPARATION MAP): `app/` (417) + `app.mjs` + `test/unit/` (196) → `junk/services/web/…`; the 7 BACKEND_ONLY modules → `junk/` whole; the 25 MIXED modules → junk ONLY `<M>/app` + `<M>/test` (KEEP `<M>/{frontend,stories,shared}` + `module.json`/`package.json`); the 2 FRONTEND_ONLY modules stay. Touch webpack/tsconfig `@modules/*`/`@wf/*` aliases only if a KEEP import pointed at a jinked path (expected: none — they point at the KEPT `frontend/` subtrees). Verify go build green + no KEEP import of a jinked path.
...
**P7.5 VERIFIED BOUNDARY (Slice A DONE, 2026-09-24):** the ENTIRE frontend/shared/types → Node-backend coupling is **type-only** — **no runtime value import**. The shared types are **relocated (Slice A, commit `bccf426d5e`)** into the kept `services/web/types/api/` home, with re-export shims at the old (junk-bound) paths:
- `Tag` — was `app/src/Features/Tags/types.d.ts` (7 lines, self-contained), 22 frontend importers → now `types/api/tags.d.ts` (shim at old path).
- `Source`/`PrivilegeLevel`/`PublicAccessLevel` (+ `isPrivilegeUpgrade` type decl) — was `app/src/Features/Authorization/types.d.ts` (33 lines) → now `types/api/authorization.d.ts` (shim); the `isPrivilegeUpgrade` **runtime impl** is `PrivilegeLevels.mjs` (Node backend, untouched).
- notification prefs schemas — was `modules/notifications/app/src/types.d.ts` (+ `export {}` `.js` stub) → now `types/api/notifications.d.ts` (shim; the `.js` stub stays).
- `SharingPermissions` — the module `modules/sharing-permissions` **does not exist in the CE tree**; the two importers (`types/user.ts`, `frontend/js/utils/meta.ts`) are pre-existing, type-only, build-erased, unresolvable refs (a SaaS module) — **not** a real coupling, out of scope.
All other kept-tree `app/` refs are **comments**, not dependencies. **Result: the Node backend (`app/` 417, `app.mjs`, `test/unit` 196, per-module `app/`/`test/`) has ZERO frontend runtime/type deps and is cleanly junk-able.** Oracle stays safe (current e2e image still serves Node from the old path; preservation is a build-time choice at step E, not a consequence of the repo move).

### P7.5 MODULE SEPARATION MAP (CRITICAL scope correction, 2026-09-24)

**Correction:** `modules/` is a MIX of Node backend **and** per-module frontend — it is NOT the "34 SaaS/CE-unused, junk-whole" the earlier spec assumed. `tsconfig.json` aliases `@modules/*`→`./modules/*` and `@wf/*`→`./modules/writefull/frontend/js/integration/src/*`, and its include set is `modules/**/frontend/js/**/*.*`, `modules/*/shared/**`, `modules/**/test/frontend/**`, `modules/**/stories/**` — so **per-module frontends are part of the KEPT frontend build**; only the per-module **backends** (`<M>/app`, `<M>/test`) are junk.

Definitive classification (generated from `git ls-files`):
- **BACKEND_ONLY (7) → junk whole:** `authentication`, `history-v1`, `notifications`, `registration-page`, `sandboxed-compiles`, `server-ce-scripts`, `track-changes`. (Note: `notifications`' shared types were relocated in Slice A, so it is now purely backend.)
- **MIXED (25) → junk ONLY `<M>/app` + `<M>/test`; KEEP `<M>/{frontend,stories,shared}` + `module.json`/`package.json`:** `admin-tools`, `bib-editor`, `ce-ui`, `diagram`, `dropbox`, `full-project-search`, `git-bridge`, `github-sync`, `instance-stats`, `languagetool`, `latex-editor`, `launchpad`, `llm`, `mendeley`, `ollitex-hub`, `orcid-picker`, `page-shells`, `python-runner`, `template-gallery`, `tex-autoformatter`, `toast-image`, `typst`, `user-activate`, `webdav`, `zotero`. (Of these, `ce-ui`, `diagram`, `full-project-search`, `latex-editor`, `python-runner`, `toast-image` have no `app/` → effectively keep whole.)
- **FRONTEND_ONLY (2) → keep whole:** `reference-picker`, `symbol-palette`.

Regenerate the definitive list before executing step B (machine-derived, not hand-typed): `for M in $(git ls-files services/web/modules | sed 's#services/web/modules/##;s#/.*##' | sort -u); do fe=$(git ls-files "services/web/modules/$M/frontend" "services/web/modules/$M/stories" "services/web/modules/$M/shared" | grep -cz ''); printf '%-20s %s\n' "$M" "$([ $fe -gt 0 ] && echo 'KEEP-frontend(junk app/test)' || echo 'JUNK-whole')"; done`
