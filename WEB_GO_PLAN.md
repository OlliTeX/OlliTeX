...
⇒ The api-profile surface is **NOT 100%**: the TPDS sync + project-new/resolve/folder-update slice is complete (+ gated), `GET /perfTest` + `GET /internal/project/:id` + `POST /internal/project/:id/deactivate` are DONE+gated, and editor `GET doc/:doc_id` + `POST .../changes/reject` are DONE+gated (below); the /internal/* destructive×6 + editor `join` + `history/resync` rows above remain. Do NOT flip web-api-overleaf until they are 1:1 (plus the web valid-basic gap).
