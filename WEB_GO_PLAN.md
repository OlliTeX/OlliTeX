...
- Editor privateApi **remaining**: POST /project/:pid/history/resync (Node 401 / Go 404) — document-updater cross-service history business logic (not a simple web handler; ownership unverified). (`GET /project/:pid/doc/:doc_id` **already DONE+gated**; `POST .../doc/:doc_id/changes/reject` **DONE+gated**; **`POST /project/:pid/join` NOW DONE+gated** — below.)
- `/status` ✓ (Go "web is alive (api)" == Node), `/health_check/{redis 200, mongo 500}` ✓ (both match).
⇒ The api-profile surface is **NOT 100%**: the TPDS sync + project-new/resolve/folder-update slice is complete (+ gated), `GET /perfTest` + `GET /internal/project/:id` + `POST /internal/project/:id/deactivate` are DONE+gated, and editor `GET doc/:doc_id` + `POST .../changes/reject` + **`POST .../join`** are DONE+gated (below); the /internal/* destructive×6 + editor `history/resync` rows above remain. Do NOT flip web-api-overleaf until they are 1:1 (plus the web valid-basic gap).
