...
// 2026-09-16 (owner #10): the E2E stack enforces SANDBOXED tex compiles
// (clsi DockerRunner; the local runner and the local TeX Live install are
// gone from OlliTeX). The DockerRunner SAME-PATH contract: clsi must sync
// project files to HOST directories that the compile sibling container
// (texlive/texlive:latest-full, uid 33) mounts at /compile.
//
// 2026-09 (PDF-404 root fix): these MUST be the reference paths
// /var/lib/overleaf/data/{compiles,cache,output} — the image's baked
// clsi-nginx.conf SERVES the compile OUTPUT from exactly that root
// (root /var/lib/overleaf/data/output/<pid>-<uid>/generated-files/<bid>/).
// The previous ./local-data/texlive/* redirect (which this override added
// on top of the env) made every editor PDF + template-bundle PDF 404. The
// compose bind-mounts the host dirs at the identical container paths.
const E2E_HOST = '/var/lib/overleaf/data'
settings.path.compilesDir = E2E_HOST + '/compiles'
settings.path.clsiCacheDir = E2E_HOST + '/cache'
settings.path.outputDir = E2E_HOST + '/output'
