/**
 * B2 (GO_CUTOVER_PLAN.md) — linked-url-proxy contract gate (Go).
 *
 * The contract was pinned 1:1 from the Node sources (services/linked-url-proxy,
 * strict-url-sanitise@0.0.1, als-normalize-urlpath@2.3.0, libraries/fetch-utils)
 * and proven Node==Go 21/21 on 2026-09-16 (GO_CUTOVER_PLAN record). The Node
 * implementation has since been REMOVED from the tree (Go is canonical), so
 * this spec now drives the deterministic battery against Go on both stacks:
 * battery = `tests/e2e/parity/lup/battery.js` (21 cases: health, method/path
 * 404s with the EXACT Express page, missing-param 400, invalid-URL 500s with
 * the exact `Invalid url to pass to open(): <raw>` messages, blocked-IP 403s,
 * DNS 421, upstream 200/404/redirect/413/422/408 cases, >2000-char path 400).
 *
 * Legs (serial, offline):
 *   1. GO (e2e)        on the disposable ol-e2e stack (fresh repo binary)
 *   2. GO (LIVE)       on `overleafserver` (the serving instance) — then the
 *                      loopback upstream + allowed-resources escape hatch
 *                      are cleaned up.
 *
 * Both legs must be 21/21. Upstream-dependent cases use a loopback-only test
 * upstream (127.0.0.1:9991) exempted via OVERLEAF_LINKED_URL_ALLOWED_RESOURCES;
...
const GO_BIN = path.join(REPO_ROOT, 'bin', 'linked-url-proxy')
...
function ensureGoBin() {
  ...
  try {
    ...
    expect(existsSync(GO_BIN), `Go binary missing: build it with \`cd ${REPO_ROOT} && go build -o bin/linked-url-proxy ./cmd/linked-url-proxy\``).toBeTruthy()
  }
}
...
test('1: GO contract battery — 21/21 (ol-e2e stack)', () => {
  const c = e2eContainer()
  expect(c, 'ol-e2e container running').toBeTruthy()
  ensureGoBin()
  dcp(GO_BIN, c, '/usr/local/bin/go-services/linked-url-proxy')
  provision(c)
  setProxyMode(c, true) // Go-only run scripts ignore the flag; legacy images use it.
  const out = runBattery(c)
  expect(out, out).toContain(EXPECTED)
})

test('2: GO contract battery — 21/21 (LIVE overleafserver), then clean test artifacts', () => {
  const c = 'overleafserver'
  const up = execFileSync('docker', ['ps', '--filter', `name=${c}`, '--format', '{{.Names}}'], { encoding: 'utf8' }).trim()
  if (up !== c) {
    test.skip(true, `container ${c} not running — live leg skipped (e2e leg still gates)`)
    return
  }
  ensureGoBin()
  dcp(GO_BIN, c, '/usr/local/bin/go-services/linked-url-proxy')
  ...
  dexe(c, `rm -f ${BATTERY_ENV_FILE} ${FLAG_FILE}; sv down lup-upstream 2>/dev/null || true; rm -rf /etc/service/lup-upstream; sv restart ${PROXY_SVC}`)
...
test.afterAll(() => {
  // Leave ol-e2e clean (batteries re-provision each run) with the loopback
  // test upstream stopped; live keeps serving Go.
  const c = e2eContainer()
  if (!c) return
  try {
