...
describe('CodeMirror EditorFacade', function () {
  ...
  it('attach/detach are honest no-ops now that OT is retired (D25/F2)', function () {
    const editor = new EditorFacade(view)
    const shareDoc = {
      on: sinon.stub(),
      getText: sinon.stub().returns('something nice'),
    ...
    editor.detachShareJs()

    // The Yjs engine owns sync; the OT share doc is never wired up.
    expect(shareDoc.on.callCount).to.equal(0)
  })
})
