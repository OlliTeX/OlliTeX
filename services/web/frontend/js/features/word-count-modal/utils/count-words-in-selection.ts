/**
 * 2026-09 (OlliTeX, owner batch: selected-text word count): word counting for
 * an editor SELECTION — complements the whole-document count in File → Word
 * count (reviewer guidance: always show the document total; add the
 * selection total when text is selected).
 *
 * Ported from the community contribution
 *   https://github.com/hnd0hng/overleaf-cep/tree/feature/selected_text_word_count
 * (services/web/modules/selected-word-count/frontend/utils/count-words-in-selection.ts)
 * — see CREDITS.md. Import paths adapted to this tree.
 */
