...
const DockerRunner = {
  run(
    ...
    const name = `typst-project-${projectId}-${fingerprint}`
    options.name = name
  ...
  _getContainerOptions(
    ...
    const options = {
      // pandoc/typst image has its own ENTRYPOINT (/usr/local/bin/pandoc); clsi_typst
      // always calls the binary directly, so clear it (verified: exit 1 with default).
      Entrypoint: [],
      Cmd: command,
      Image: image,
      WorkingDir: cwd
  ...
  destroyOldContainers(callback) {
    dockerode.listContainers({ all: true }, (error, containers) => {
      ...
      for (const container of containers) {
        ...
        if (name.slice(0, 14) === '/typst-project-' && ttl <= 0) {
          // extract projectId: /project-<projectId>-...
          const match = name.match(/^\/typst-project-([a-f0-9]{24})-/)
